...
def read_func_mem_size(name):
    ...
    if base_name not in app:
        print(f"Warning: '{base_name}' not found in function_mem.csv")
    return app.get(base_name, 0)
...
def compute_exe(app, server_pair,CI):
    ...
    # read app data
    data_app = app.split('_')[0]
    with open(f'{Path(__file__).parents[0]}/data/avg_data/{data_app}_{old}.metal.json','r') as old_data_file:
        old_data = json.load(old_data_file)[0]
    with open(f'{Path(__file__).parents[0]}/data/avg_data/{data_app}_{new}.metal.json','r') as new_data_file:
...
def compute_kat(app, server, kat, CI):
    ...
    # read app data
    data_app = app.split('_')[0]
    with open(f'{Path(__file__).parents[0]}/data/avg_data/{data_app}_{server}.metal.json','r') as data_file:
        server_data = json.load(data_file)[0]
